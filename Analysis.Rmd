...
ggplot(dummyData, aes(x = incubationTime_d, y = k_C_d)) +
  geom_line(aes(color = incubat))
...
kruskal.test(O_C ~ DOMSource, data = massSpec)
kruskal.test(H_C ~ DOMSource, data = massSpec)
...
a <-
  ...
  geom_jitter(data = deltaDOC %>% filter(Enrichment != "CTRL microbes"), 
              aes(x = DOMSource, y = deltaDOC_mgCL, shape = Enrichment, 
                  color = DOMSource), size = 2.5, width = 0.4) +
  # Summary points
  ...
  geom_point(data = deltaDOC_summary, 
             aes(x = DOMSource, y = deltaDOC_mgCL_mean),
             size = 2, shape = 16, color = "white") +
...
biodegrad_crossing_interp_summary <- 
  ...
  summarise(biodegrad_perc_mean = mean(biodegrad_perc),
            ...
            k_C_d_mean = mean(k_C_d),
            k_C_d_sd = sd(k_C_d)) %>%
  mutate(facetCols = factor(paste0(Time_hrs/24, " day(s)"),
...
b <-
  ...
  geom_point(data = biodegrad_crossing_interp_summary, 
             aes(x = DOMSource, y = biodegrad_perc_mean),
             size = 2, shape = 16, color = "white") +
  # Shape and color scales
...
c <-
  ...
  geom_jitter(data = biodegrad_crossing_interp %>% 
                filter(Time_hrs %in% c(24, 72, 7*24, 10*24) &
                         Enrichment != "CTRL\nmicrobes"), 
              aes(x = DOMSource, y = k_C_d, 
                  shape = Enrichment, color = DOMSource), 
              size = 2.5, width = 0.4) +
  # Summary points
  geom_linerange(data = biodegrad_crossing_interp_summary, 
             aes(x = DOMSource, 
                 ymin = k_C_d_mean - k_C_d_sd,
                 ymax = k_C_d_mean + k_C_d_sd,
                 color = DOMSource),
             linewidth = 4) +
  geom_point(data = biodegrad_crossing_interp_summary, 
             aes(x = DOMSource, y = k_C_d_mean),
             size = 2, shape = 16, color = "white") +
  # Shape and color scales
  scale_color_manual(values = c("CHLOR" = "#D84727", 
                                "ES" = "#FFB000",
...
layoutGrid <- rbind(c(1,1,2,2,2,2,2),
                    c(1,1,3,3,3,3,3))
...
ggsave(file = "figures/Figure_4.png", width = 6, height = 6, 
       dpi = 300, units = "in")
```
...
c <- 
  ...
  # Overlay points
  geom_jitter(aes(color = System), width = 0.25, shape = 16, alpha = 0.75) +
...
d <- 
  ...
  geom_point(aes(color = System), alpha = 0.75, shape = 16) +
  # Trend line
  geom_smooth(data = synthesis_fw,
              aes(group = System, color = System, fill = System), 
              method = "lm", fullrange = TRUE, alpha = 0.25) +
  stat_regline_equation(aes(color = System), size = 2.5, 
                        label.x = log10(0.02), 
                        label.y = c(log10(0.75), log10(0.375))) +
...
e <- 
  ...
  stat_regline_equation(size = 2.5, 
                        label.x = log10(0.02), 
                        label.y = c(log10(0.0018), log10(0.0005)),
  ...
  stat_cor(aes(color = System, label = paste(after_stat(rr.label), 
           ...
           size = 2.5, 
           label.x = log10(0.02), 
           label.y = c(log10(0.001), log10(0.0003)),
           p.accuracy = 0.001, r.accuracy = 0.001) +
  
  scale_fill_manual(values = c("#D84727","#2E4756")) +
  scale_color_manual(values = c("#D84727","#2E4756")) +
  scale_x_log10() +
  scale_y_log10(limits = c(0.00025, 20), breaks = c(1E-3, 1E-2, 1E-1, 1E0, 1E1),
...
# Number of incubation experiments
length(unique(synthesis_fw$Sample))

# How many lakes and rivers
synthesis_fw %>% 
  select(Citation, System, Site, Sample) %>%
...
synthesis_fw %>%
  group_by(Citation, System, Site, Sample) %>% 
  summarise(minTime = min(incubationTime_d)) %>%
  filter(minTime <= 1)

# Most common time points for measurement: different between groups?
t.test(incubationTime_d ~ System, data = synthesis_fw)

# Most common measurement times
synthesis_fw %>%
  ungroup() %>%
  transmute(incubationTime_d = round(incubationTime_d)) %>%
...
t.test(maxTime ~ System, data = maxDur)
...
synth_initDOC <- 
  synthesis_fw %>%
  ungroup() %>%
  select(Citation, System, Site, Sample, DOCi_mgL) %>%
  distinct() 
# Overall mean
synth_initDOC %>%
  summarise(mean(DOCi_mgL), sd(DOCi_mgL),
            min(DOCi_mgL), max(DOCi_mgL))
# Differences between Systems?
t.test(DOCi_mgL ~ System, data = synth_initDOC)
...
modl <- lm(log10(BDOC_perc) ~ log10(incubationTime_d), data = synthesis_fw)
summary(modl)

# Difference in biodegradability between habitats
t.test(BDOC_perc ~ System, data = synthesis_fw)
synthesis_fw %>%
  ungroup() %>% group_by(System) %>%
  summarise(mean(BDOC_perc), sd(BDOC_perc),
            mean(k_day), sd(k_day))

# difference in removal rate between habitats
t.test(k_day ~ System, data = synthesis_fw)
```
