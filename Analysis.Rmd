...
# if concentration is less than detection, set to 0 for now
...
```{r crossing-biodegradeCremovalCalc}
...
rm(nut_crossingWide)
nutInits$Experiment <- str_extract(nutInits$Experiment, "\\d")

### DOC biodegradability calculations ###
# Grab gas data for crossing experiment
gas_crossing <-
  gas %>% 
  filter(Experiment %in% c("DOM Crossing Experiment 1", 
                           "DOM Crossing Experiment 2")) %>%
...
# join with info about final DOC concentration in replicate
#finalDOC <- 
  #nut_crossing %>%
  #filter(BlankOrSample == "sample" & DayOfExperiment == "final" &
  #         Analysis == "DOC_mgCL") %>%
  #mutate(Enrichment = as.factor(paste0(Enrichment, "\nmicrobes")),
   #      DOMSource = as.factor(paste0(DOMSource, "\nDOM source")),
   #      Replicate = as.factor(Replicate)) %>%
#  select(Enrichment, DOMSource, Replicate, Concentration) %>%
 # rename(final_DOC_mgCL = Concentration)
#biodegrad_crossing <-
 # full_join(biodegrad_crossing, finalDOC, relationship = "many-to-many")
...
# Interpolate rates if pulled on certain days: 1, 3, 7, 10
biodegrad_crossing <- 
  biodegrad_crossing %>%
...
```{r crossing-C summary statistics}
# Summary of initial DOM and CO2 accumulation at time points
...
```{r, Fig2 initial DOM characteristics}
#### Mass spec #################################################################
massSpec_CHLOR_Control <- 
  read_csv("data/CHLOR Control_100-800_Assigned.csv") %>%
  mutate(DOMSource = "CHLOR")

massSpec_ES_Control <- read_csv("data/ES Control_100-800_Assigned.csv") %>%
  mutate(DOMSource = "ES")

massSpec_SRFA_Control <- read_csv("data/SRFA Control_100-800_2_Assigned.csv") %>%
...
# Factorize groups
massSpec$group <- factor(massSpec$group, level = c("CHO", "CHNO", "CHOS", "CHNOS"))
massSpec$DOMSource <- factor(massSpec$DOMSource, level = c("SRFA", "ES", "CHLOR"))

a <- 
  ggplot(data = massSpec, aes(x = O_C, y = H_C)) +
  facet_wrap(vars(DOMSource)) +
  geom_point(aes(color = group), size = 0.5) +
  #geom_point(aes(color = exp_mass), alpha = 0.5) +
  scale_color_manual(values = c("#D84727","darkgrey", "#FFB000","#2E4756")) +
  # Theme adjustments
  theme_minimal() + labs(tag = "a", color = "Group") +
  guides(colour = guide_legend(override.aes = list(size = 2))) +
  xlab("Oxygen:Carbon") + ylab(expression("Hydrogen:Carbon")) +
  #scale_shape_manual(values = c(21, 24)) +
  theme(axis.text = element_text(color = "black", size = 8),
        axis.title = element_text(color = "black", size = 10),
        panel.spacing = unit(1.5, "lines"), 
        plot.background = element_rect(fill = "white", color = NA),
        legend.position = c(0.95, 0.2), plot.margin = unit(c(0,0,0,0), "cm"),
...
#### EEMs ######################################################################
eem$DOMSource <- factor(eem$DOMSource, level = c("SRFA", "ES", "CHLOR"))
...
ggsave(grob, file = "figures/Enrichment_EEMs_vanKrev_T0.png", 
       width = 7, height = 5, 
       units = "in", dpi = 300)
```

```{r Fig3 compare biodegradability and removal rate, fig.height=6.5, fig.width=7.5, fig.cap="Comparison of change in DOC concentration (ΔDOC, mg-C L-1) from start to end of the experiment (a, where negative numbers represent DOC degradation) and C biodegradability (%, b), C removal rate (d-1, c) on incubation day 1, 3, 7, and 10. Shapes = microbial enrichment, where circles = Suwannee River fulvic acid (SRFA), triangles = Elliott soil (ES), and squares = Chlorella (CHLOR) enrichment. Diamond outlined in black represents the median for each DOM source, with standard error denoted by error bars. Statistics (a, b, c) are p-values from t-tests comparing group mean differences."}
selectTimes <-
  biodegrad_crossing_interp %>% 
...
deltaDOC$Enrichment <- factor(deltaDOC$Enrichment, 
                             levels = paste0(c("SRFA", "ES", "CHLOR"), 
                                             " microbes"))
deltaDOC$Time_hrs <- "14 days"

a <-
  ggplot(data = deltaDOC %>% filter(Enrichment != "CTRL\nmicrobes"), 
       aes(x = DOMSource, y = deltaDOC_mgCL)) +
  facet_grid(rows = vars(Time_hrs))+
  # Overlay points
  geom_jitter(aes(shape = Enrichment, color = DOMSource), size = 2.5,
              width = 0.25) +
  ...
  stat_compare_means(aes(label = after_stat(p.signif)), method = "t.test", 
                     ref.group = "SRFA", label.y = 1, size = 4) +
...
b <-
  ...
  facet_grid(rows = vars(factor(paste0(Time_hrs/24, " day(s)"),
  ...
  stat_summary(aes(fill = DOMSource), geom = "pointrange", fun.data = mean_se,
               size = 0.75, shape = 23) +
  # Stats
  stat_compare_means(size = 2.5,label.y = 55, method = "anova") +
  stat_compare_means(aes(label = after_stat(p.signif)), method = "t.test", 
                     ref.group = "SRFA", label.y = 48, size = 4) +
  # Shape and color scales
  scale_fill_manual(values = c("CHLOR" = "#D84727", "ES" = "#FFB000",
                               "SRFA"="#2E4756", "CHLOR\nmicrobes" = "#D84727",
                               "ES\nmicrobes" = "#FFB000", 
  ...
  theme(axis.text = element_text(color = "black", size = 8),
        panel.spacing = unit(0.05, "lines"), legend.position = "none",
        strip.text = element_text(size = 10),
...
c <-
  ...
  stat_summary(aes(fill = DOMSource), geom = "pointrange", fun.data = mean_se,
               size = 0.75, shape = 23) +
  # Stats
  stat_compare_means(size = 2.5,label.y = 0.17, method = "anova") +
    stat_compare_means(aes(label = after_stat(p.signif)), method = "t.test", 
                     ref.group = "SRFA", label.y = 0.15, size = 4) +
  # Shape and color scales
  scale_fill_manual(values = c("CHLOR" = "#D84727", "ES" = "#FFB000",
                               "SRFA"="#2E4756", "CHLOR\nmicrobes" = "#D84727",
                               "ES\nmicrobes" = "#FFB000", 
  ...
  theme(axis.text = element_text(color = "black", size = 8),
        panel.spacing = unit(0.05, "lines"), legend.position = "none",
        strip.text = element_text(size = 10),
        plot.margin = unit(c(0.05, 0.05, 0.05, 0), "in"),
        legend.background = element_rect(color = "black"))

layoutGrid <- rbind(c(1,2,3),
                    c(1,2,3),
                    c(1,2,3))
grob <- grid.arrange(a, b, c, layout_matrix = layoutGrid)
ggsave(grob, file = "figures/Figure3.png", 
       height = 6.5, width = 7.5, units = "in", dpi = 300)
```

```{r Fig4 removal rate flannel, fig.width=5, fig.height=5, fig.cap="Carbon (C) removal rate (d-1) of each experimental replicate (n=6 per treatment). Dashed grey lines mark elapsed time of 1, 3, 7, and 10 days."}
### Plot #######################################################################
# C removal rate
...
ggsave(file = "figures/Crossing_Cremovalrate.png", width =5, height = 5, 
       dpi = 300, units = "in")
```

```{r Fig5 synthesis, fig.width=7.25, fig.height = 4.75, fig.cap="Literature synthesis of lake (red) and river (blue) C degradation incubations showing differences in (a) times of sample collection, (b) maximum incubation time, (c) ΔDOC (mg-C L-1) between start to end of incubation period, (d) change in C biodegradability (%) and (e) C removal rates (d-1) with incubation time. Vertical dashed lines in (a) and (b) mark day 1, 3, 7, and 14 of incubations. Initial (time = 0) samples excluded from (a). Negative values in (c) represent C degradation, diamonds mark means, bars mark standard error. Note that both x and y-axis in (d) and (e) are log scaled."}
...
synthesis_filt <- 
  ...
  filter(BDOC_perc > 0)
# Identify samples that now only have one observation
dropSynth <- 
  synthesis_filt %>%
  count(Sample) %>%
  filter(n == 1) %>%
  select(-n)
# Remove samples that only have one observation
synthesis_filt <- 
...
# Plot observation counts of incubation lengths
a <- 
...
ggsave(grob, file = "figures/Synthesis_Cdecay.png", width = 7.25, height = 4.75, 
       units = "in", dpi = 300)
```
...
table2 <- 
  ...
  filter(Time_d %in% c(1, 3, 7, 13)) %>%
  summarise(CO2_mgCL = mean(CO2Accum_mgCL, na.rm = T),
            sdCO2 = sd(CO2Accum_mgCL, na.rm = T)) %>%
  mutate(CO2_mgCL = paste0(round(CO2_mgCL, digits = 3), " (", 
                           round(sdCO2, digits = 3), ")")) %>%
  select(-sdCO2) %>%
...
nut_final_crossing %>%
  ungroup() %>% group_by(DOMSource, Enrichment) %>%
  summarise(mean(final_DOC_mgCL), sd(final_DOC_mgCL))
```

```{r FigS2 CO2 accumulation rate flannel, fig.width=5, fig.height=5, fig.cap="CO2 accumulation rate (as mg-C L-1 min-1) in each experimental replicate (n=6 per treatment). Dashed grey lines mark elapsed time of 1, 3, 7, and 10 days."}
### Plot #######################################################################
ggplot(data = biodegrad_crossing %>% filter(Enrichment != "CTRL\nmicrobes")) +
  # Facet based on DOM source and microbe addition
  facet_grid(cols = vars(DOMSource),
...
# log-log regression
modl <- lm(log10(CO2Rate_mgCLmin) ~ log10(Time_hrs), data = avgPeakRate)
summary(modl)
...
# What was CO2 accumulation rate dependent on?
modl <- aov(CO2Rate_mgCLmin ~ DOMSource * Enrichment * Time_d, 
            data = biodegrad_crossing_filt)
summary(modl)
# Signif interaction between DOM source and Time
modl <- aov(CO2Rate_mgCLmin ~ DOMSource * as.factor(Time_d), 
            data = biodegrad_crossing_treatmentMeans)
summary(modl)
TukeyHSD(modl)
# Log-log regression for time
plot(y = biodegrad_crossing_treatmentMeans$CO2Rate_mgCLmin, 
     x = biodegrad_crossing_treatmentMeans$Time_d)

modl <- 
  lm(log10(CO2Rate_mgCLmin) ~ log10(Time_d), 
   data = biodegrad_crossing_treatmentMeans)
summary(modl)
...
```{r FigS3 CO2 accumulation plot, fig.width=5, fig.height=5, fig.cap="CO2 accumulation (as mg-C L-1) in each experimental replicate (n=6 per treatment). Dashed grey lines mark elapsed time of 1, 3, 7, and 10 days."}
ggplot(data = biodegrad_crossing %>% filter(Enrichment != "CTRL\nmicrobes")) +
  # Facet based on DOM source and microbe addition
  facet_grid(cols = vars(DOMSource),
             rows = vars(Enrichment)) +
...
```{r CO2 rate stats}
### Rates at 1, 3, 7, 13 days ##################################################
# What was co2 accumulation impacted by?
...
```{r FigS4 biodegradability, fig.width=5, fig.height=5, fig.cap="Carbon (C) biodegradability (%) of each experimental replicate (n=6 per treatment). Dashed grey lines mark elapsed time of 1, 3, 7, and 10 days."}
## Plot ########################################################################
...
```{r supplemental figure CTRL results}
...
```{r decay rate stats}
## Rates at 1, 3, 7, 13 days ###################################################
# What was C decay rate impacted by?
modl <- aov(k_C_d ~ DOMSource * Enrichment * Time_d, 
            data = biodegrad_crossing_filt)
summary(modl)
TukeyHSD(modl)
# Significant interaction between dom source & enrichment
modl <- aov(k_C_d ~ DOMSource * Enrichment, 
            data = biodegrad_crossing_filt)
summary(modl)
TukeyHSD(modl)
# Significant interaction between dom source & time
modl <- aov(k_C_d ~ DOMSource * as.factor(Time_d), 
            data = biodegrad_crossing_filt)
summary(modl)
TukeyHSD(modl)
...
```{r}
select <- biodegrad_crossing_interp %>% 
  filter(Time_hrs %in% c(24, 72, 7*24, 13*24))
kruskal.test(k_C_d ~ Time_hrs, data = select)

### Review what C decay and biodegradability were both impacted by #############
...
min(biodegrad_crossing_filt$biodegrad_perc)

skewness(log())
ggplot(data = biodegrad_crossing_filt) +
  geom_histogram(aes(x = log10(CO2Rate_mgCLmin + abs(min(CO2Rate_mgCLmin))))) 

biodegrad_crossing_filt$CO2Rate_mgCLmin+abs(min(biodegrad_crossing_filt$CO2Rate_mgCLmin))

# co2 accumulation

qqnorm(biodegrad_crossing_filt$biodegrad_perc)
qqline(biodegrad_crossing_filt$biodegrad_perc)

# Compute skewness

skewness(biodegrad_crossing_filt$biodegrad_perc)

# Totally non normal, need to correct
# See if log10 fixes
skewness(log10(biodegrad_crossing_filt$biodegrad_perc+10))
# Nice, skewness reduced from 1.13 to 0.32!
biodegrad_crossing_filt <- mutate(biodegrad_crossing_filt, 
                                  biodegrad_perc_log10 = log10(biodegrad_perc+10))
...
summary(modl)
TukeyHSD(modl)
modl <- aov(biodegrad_perc ~ Enrichment * as.factor(Time_d), 
            data = biodegrad_crossing_filt)
summary(modl)
TukeyHSD(modl)
# C decay
